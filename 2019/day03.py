...
def convert_moves(x):
    ...
    for i in m:
        ...
        if direction == "R":
            moves.append(complex(distance))
        if direction == "L":
            moves.append(complex(-distance))
        if direction == "U":
            moves.append(complex(0, distance))
        if direction == "D":
            moves.append(complex(0, -distance))
    return moves
...
def to_path(moves):
    ...
    for move in moves:
        if move.imag == 0:
            move_int = int(move.real)
            pos_neg = int(move_int / abs(move_int))
            cur_pos = positions[-1]
            (x, y) = (int(cur_pos.real), int(cur_pos.imag))
            for i in range(x + pos_neg, x + move_int + pos_neg, pos_neg):
                positions.append(complex(i, y))
        else:
            move_int = int(move.imag)
            pos_neg = int(move_int / abs(move_int))
            cur_pos = positions[-1]
            (x, y) = (int(cur_pos.real), int(cur_pos.imag))
            for i in range(y + pos_neg, y + move_int + pos_neg, pos_neg):
                positions.append(complex(x, i))
    return positions
...
for overlap in intersection(res1, res2):
    if overlap == 0:
        ...
        wiredist = res1.index(overlap) + res2.index(overlap)
        if mindist is None:
            mindist = wiredist
        elif wiredist < mindist:
            mindist = wiredist
print(mindist)
