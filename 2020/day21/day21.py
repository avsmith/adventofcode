...
for ingredient in poss_menu_allergens:
    overlap = list(
        reduce(lambda i, j: i & j, (set(x) for x in poss_menu_allergens[ingredient]))
    )
...
allergen_menu_items = list(
    itertools.chain.from_iterable([v for v in reduced_menu_allegens.values()])
)
...
def find_allergens(menu_possibilities):
    menu_stuff = list(
        itertools.chain.from_iterable([v for v in menu_possibilities.values()])
    )
