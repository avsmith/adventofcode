                ...
                if searchnum in set(tile_bag[tilenum][orient]):
                    ...
                    for i in range(len(tile_sides)):
                        ...
                        if possible[desired_side] == searchnum:
                            tile_text = tile_print(
                                tile_bag[tilenum]["tile"], orient, -i
                            )
                            return [[tilenum], possible, [i], [orient], tile_text]
...
def draw_monster(string, match):
    for matchnum in range(1, len(match.groups()) + 1):
        for matchpos in range(match.start(matchnum), match.end(matchnum)):
            string = "".join(
                [
                    "O" if charpos == matchpos else letter
                    for charpos, letter in enumerate(string)
                ]
            )
    return string
...
for tilenum in tile_bag:
    if (
        len([side_counts[x] for x in tile_bag[tilenum]["clock"] if side_counts[x] == 1])
        == 2
    ):
    ...
    for j in range(side_size):
        if i == 0 and j == 0:
            ...
            if j == 0:
                ...
                pos = i * side_size + j
                current_tile = find_tile(
                    all_tiles[pos - 1][1][1], all_tiles[pos - 1][0][0], 3
                )
                all_tiles.append(current_tile)
                current_tile_text = current_tile[4]
                ...
                for k in range(rows):
                    tile_text[k + i * (rows)] = (
                        tile_text[k + i * (rows)] + current_tile_text[k]
                    )
...
for i in range(4):
    ...
    for j in range(1, len(tr) - 1):
        for match in monstre2.finditer(tr[j]):
            ...
            if monstre1.fullmatch(tr[j - 1], s, e) and monstre3.fullmatch(
                tr[j + 1], s, e
            ):
                count += 1
    ...
    for j in range(1, len(tt) - 1):
        for match in monstre2.finditer(tr[j]):
            ...
            e = match.end()
            #      print ('String match "%s" at %d:%d' % (tr[j][s:e], s, e))
            if monstre1.fullmatch(tt[j - 1], s, e) and monstre3.fullmatch(
                tt[j + 1], s, e
            ):
                count += 1
