...
for param in params.splitlines():
    ...
    param_dict[ticket_category] = list(range(int(x_min), int(x_max) + 1)) + list(
        range(int(y_min), int(y_max) + 1)
    )
...
for nearby in nearby_tix.splitlines():
    ...
    invalid_nums = np.setdiff1d(
        nearby_nums, list(itertools.chain.from_iterable(param_dict.values()))
    )
    if len(invalid_nums) > 0:
        ...
        for i in range(len(nearby_nums)):
            possible_values = [
                list(param_dict.keys())[j]
                for j, elem in enumerate(list(param_dict.values()))
                if nearby_nums[i] in elem
            ]
...
while any(len(elem) > 1 for elem in ticket_rules):
    determined_values = set(
        list(
            itertools.chain.from_iterable(
                [
                    ticket_rules[i]
                    for i, elem in enumerate(ticket_rules)
    ...
    for i in range(len(ticket_rules)):
        if len(ticket_rules[i]) > 1:
            ticket_rules[i] = [
                rule for rule in ticket_rules[i] if rule not in determined_values
            ]
